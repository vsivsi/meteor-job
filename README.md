...
Meteor Job is a pure Javascript implementation of the `Job` and `JobQueue` classes that form the foundation of the [`jobCollection` Atmosphere package](http://atmospherejs.com/package/jobCollection) for Meteor. This package is used internally by `jobCollection` on Meteor, but you should also use it for any job workers you would like to run outside of the Meteor environment as ordinary node.js programs.
